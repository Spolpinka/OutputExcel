...
public class FormingComplaint {
    ...
    private String textOfComplaint(String[] line) {
        ...
        if (line[1].equals("Уведомление об отказе в предоставлении информации об исполнительном производстве")) {
            ...
            complaint.append(firstPartForNormal).append(line[2]).append(secondPart).append(accounts(line)).
                    append(placeOfWork(line)).append(transport(line)).append(estate(line)).append(lastPartForAll);
    ...
    private String transport(String[] line) {
        String s = "";
        int blockOfTransport = Integer.parseInt(line[32]);
        String transport = line[44];//описание ТРАНСПОРТА
        String[] realisation = {line[34] //Акт о наложении ареста (описи имущества)
                , line[35] // Предложение взыскателю оставить не реализованное в принудительном порядке имущество за собой
                , line[36] // Уведомление о нереализации имущества на комиссионных началах
        ...
        if (transport != null && transport.length() > 10 && blockOfTransport == 0) {
    ...
    private String estate(String[] line) {
        String s = "";
        String estate = line[46];//сведения о НЕДВИЖКЕ
        String[] realisation = {line[34] //Акт о наложении ареста (описи имущества)
                , line[35] // Предложение взыскателю оставить не реализованное в принудительном порядке имущество за собой
                , line[36] // Уведомление о нереализации имущества на комиссионных началах
                , line[37]}; //Акт возврата арестованного имущества с реализации
        ...
        if (estate != null && estate.length() > 10 && line[33] == null) {
        ...
        } else if (estate != null && estate.length() > 10 && Integer.parseInt(line[33]) != 0 && sumOfResolutionsForRealisation == 0) {
