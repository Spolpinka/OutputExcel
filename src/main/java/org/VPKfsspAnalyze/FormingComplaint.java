...
class FormingComplaint {
    ...
    String lessThenMin = " Просим не рассматривать данный пункт жалобы в случае, если общая задолженность по исполнительному " +
            "производству составляет менее 5% от оценочной стоимости соответствующего имущества.";
    ...
    protected void analyzeForComplaint(String path, boolean isNeedComplaintFile) {
        ...
        String[][] baseOfComplaint = new String[fullbase.length][namesOfColumns.length];
        ...
        if (isNeedComplaintFile) {
            ...
            ot1.start();
            System.out.println("пошел поток записи жалоб");
        }
    ...
    private String accounts(String[] line) {
        ...
        for (String value : recovery) {
            try {
                ...
                //throw new RuntimeException(e);
                System.out.println("Ошибка парсинга числа постановлений");
            }
        }
        //смотрим были ли ненулевые счета
        int notNullAccs = 0;
        try {
            notNullAccs = Integer.parseInt(line[42]);
        } catch (Exception e) {
            System.out.println("Ошибка при парсинге количества ненулевых счетов");
        }
        if (sumOfAcc == 0) {
    ...
    private String transport(String[] line) {
        ...
        int sumIP = 0;
        try {
            sumIP = Integer.parseInt(line[62]);
        } catch (Exception e) {

        }
        if (sumIP >= 3000 || sumIP == 0) {//ограничение для небольших задолженностей, по которым не будут обращать взыскание на транспорт
            if (transport != null && transport.length() > 10 && (line[44] == null || line[44].equals("0"))) {
                return "- у должника обнаружено транспортное средство, одновременно, отсутствует Постановление " +
                        ...
                        "что может повлечь распоряжением должником своим имуществом в ущерб интересам кредиторов." + lessThenMin + "\n";
    ...
    private String estate(String[] line) {
        ...
        if (sumIP == 0 || sumIP >= 10000) {//ограничение на сумму долга, слишком маленькое для обращения взыскания
            if (estate != null && estate.length() > 10 && line[45] == null) {
                return "- у должника обнаружено недвижимое имущество, одновременно, отсутствует Постановление о " +
                        ...
                        "распоряжением должником своим имуществом в ущерб интересам кредиторов." + lessThenMin + "\n";
    ...
    private String stopDeparture(String[] line) {
        ...
        if (sumOfDebt > 30000f && sumOfStops < 1) {
            s = "- при общей сумме задолженности по исполнительному производству превышающей " +
                    ...
                    "ограничении на выезд должника из Российской Федерации (просим не рассматривать данный пункт по существу, " +
                    "если в материалах исполнительного производства отсутствуют доказательства вручения должнику Постановления " +
